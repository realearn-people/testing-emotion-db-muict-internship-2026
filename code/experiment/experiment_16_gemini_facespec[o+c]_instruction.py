...
def resolve_dataset_paths() -> tuple[Path, Path, Path]:
    """Work out where the face and spectrogram datasets live for this run.

    Reads the DATASET_ROOT environment variable (provided by Slurm) and
    appends both FACE_SUBDIRECTORY and SPECTROGRAM_SUBDIRECTORY to find
    the two folders needed to build matched face+spectrogram jobs.

    Returns:
        A (dataset_root, face_dir, spectrogram_dir) tuple of resolved
        absolute paths.
    """
    dataset_root = Path(
...
def create_config() -> PipelineConfig:
    ...
    immutable = {
        "experiment": "16.gemini-facespec[o+c]-instruction",
    ...
    return PipelineConfig(
        ...
        experiment_name="16.gemini-facespec[o+c]-instruction",
        provider=PROVIDER,
        model=MODEL,
