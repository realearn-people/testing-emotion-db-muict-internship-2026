...
def resolve_dataset_paths() -> tuple[Path, Path]:
    """Work out where the spectrogram-image dataset lives for this run.

    Reads the DATASET_ROOT environment variable (provided by Slurm) and
    appends SPECTROGRAM_SUBDIRECTORY to find the folder containing the
    labeled spectrogram images.

    Returns:
        A (dataset_root, spectrogram_dir) tuple of resolved absolute paths.
    """
    dataset_root = Path(
...
def create_config() -> PipelineConfig:
    ...
    immutable = {
        "experiment": "15.gemini-spec[o+c]-instruction",
    ...
    return PipelineConfig(
        ...
        experiment_name="15.gemini-spec[o+c]-instruction",
        provider=PROVIDER,
        model=MODEL,
